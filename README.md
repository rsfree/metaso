...
## 通路（`Authorization: Bearer` 自动判断，只有两条）

| 通路 | 触发 | 额度 | 429 恢复策略 |
|---|---|---|---|
| **登录** | Bearer 值**含 `uid=` 与 `sid=`**（即调用方自带的登录 cookie，按 cookie 缓存通路） | **登录积分 500/天**（1 发≈1 积分，首答 1~1.3s） | 等待 10s（实测恢复窗口）后**原样重试**——绝不换身份（会丢登录态） |
| **匿名** | 其余一切请求（含 `Bearer guest` / 无 token） | 匿名 ~13 发/出口/窗口 | 换身份 → 换出口 → Chrome TLS 指纹 |

节流自动调速：登录 2.5s / 匿名 15s（env 显式设置 `METASO_MIN_INTERVAL` 时以 env 为准）。
两条通路均无密钥强度——公网部署的暴露面治理交给入口层。
频控实测数字见 docs/UPSTREAM.md §11.6 追记六/七。

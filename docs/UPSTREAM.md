...
**追记七：服务化通路定稿 —— Bearer 值即凭据（BYO，2026-09-24 15:4x）**

metaso-service 的 `Authorization: Bearer` 语义收敛为两条，**自动判断、无密钥层**：

1. **登录通路**：Bearer 值含 `uid=` 与 `sid=` ⇒ 整串视为调用方自带的登录 cookie
   （按 cookie 哈希缓存通路实例，复用已热 token/会话；未带门票三件套会自动补随机指纹）。
   走登录积分 500/天 + 突发限速（4 发/突发，+10s 恢复）。
2. **未登录通路**：其余一切请求（含 `Bearer guest`、无 token、任何其他值）→ 匿名
   随机指纹身份，~13 发/出口/窗口，服务端三级恢复。

设计取舍：曾试验「METASO_API_KEYS fail-closed + env 静态 cookie 通道」，按用户决策
**移除**（API key 层与静态通道在 BYO 语义下均为死代码）——公网部署时两条通路皆
无鉴权强度，暴露面治理交给入口层。
...
**追记六：登录态频控 = 积分制（my-info 实测，2026-09-24 15:3x）**

`/api/my-info` 的 `user` 对象直接携带账本（零成本可查）：

| 字段 | 值 | 含义 |
|---|---|---|
| `todayFreePoint` | 500 | **每日免费积分** |
| `creditRest` | 497 | 当前剩余（今日 3 次登录搜索各扣 1 → 500−3=497，**精确对账**） |
| `creditTotal` | 14500 | 总积分池 |
| `metasoApiCredit` | 0 | 官方 API 额度（未购，另算） |
| `searchCount` | 0 | 与搜索次数非 1:1（口径未知） |

⇒ **登录频控 = 积分预算 + 突发限速 双层（均实测，2026-09-24 15:3x）**：
- **积分预算**：`todayFreePoint` 500/天，1 次普通搜索 ≈ 1 积分（creditRest 1:1 对账）；
- **突发限速**：~1 发/秒连打 **4 发后第 5 发帧 429**（≈4 发/短突发窗口）；**+10 秒即恢复**
  （非分钟级惩罚）——即稳态可持续 ≈ 4 发/10 秒，服务端 `METASO_MIN_INTERVAL` 可据此调到 ~2.5s；
- 登录态首答延迟 1.0~1.3s（匿名 8~11s）；
比匿名 ~13 发/窗口宽一个量级以上。匿名侧**无法仿照**
——`sid` 是服务端会话凭证（消融 401），匿名池按 IP×窗口且换身份不刷新（§11.6 追记）。

**与官方开放 API 的关系（公开资料，2026-09 检索；扣费池归属未实测）**：
官方 API 计费的也是**同一种「积分」**（搜索 10/次、网页读取 10/次、问答 18/次；
注册送 5000+2000、Bing 迁移再送 10000，充值 ≈2.6 元/千点）——`creditTotal=14500`
与注册+迁移合计 17000 量级吻合，推测**同一积分池**。字段 `metasoApiCredit=0`
疑似官方 API 专用余额（独立），即官方 API 可能要求单独开通/充值。**未实测**
（无官方 key；调一次官方 API 看扣 creditRest 还是 metasoApiCredit 即可钉死）。
网页会话搜索 1 积分/次 vs 官方 API 10 积分/次 —— 逆向路性价比 ≈ 10 倍，
但稳定性/合规性反向（无 SLA、随时可能封路）。
